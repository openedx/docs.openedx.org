...
#. Add the ``make extract_translation`` into your fork of the new repository e.g. ``Zeit-Labs/credentials`` in a new branch e.g. ``omar/add-pull-translations``
#. Fork the `openedx-translations repo`_ e.g. ``Zeit-Labs/openedx-translations``.
#. In your fork, modify the `extract-translation-source-files.yml`_ file in a new branch e.g. ``omar/add-credentials``.
#. Go to the ``Actions`` tab in your repository (e.g. ``Zeit-Labs/openedx-translations``)
...
#. Click on the "Run workflow" dropdown button with the following parameters:
   ...
   - **Use workflow from branch:** ``omar/add-credentials``
...
#. Temporarily pull translations from the fork using the Makefile command e.g. ``make ATLAS_OPTIONS="--repository=Zeit-Labs/openedx-translations --revision=omar/add-credentials" pull_translations``
...
.. _edx-platform Redwood: https://github.com/openedx/edx-platform/commit/e7fc0c6b6f8b44fa4bcc71d00ae2931f91cc664c
